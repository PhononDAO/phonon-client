...
export const PhononWallet = () => {
  ...
  // every 3 seconds we poll to see what sessions are active
  const { data } = useFetchSessionsQuery(undefined, {
    pollingInterval: 3000,
  });

  useEffect(() => {
    addCardsToState(
      data?.map((session) => convertSessionToCardSession(session) as PhononCard)
    );
