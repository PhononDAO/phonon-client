...
func (phonon *MockPhonon) Encode() (tlv.TLV, error) {
	privKeyTLV, err := tlv.NewTLV(tlv.TagPhononPrivKey, phonon.PrivateKey)
	if err != nil {
		log.Error("could not encode mockPhonon privKey: ", err)
		return tlv.TLV{}, err
	}
	//Also include CurveType
	curveTypeTLV, err := tlv.NewTLV(tlv.TagCurveType, []byte{byte(phonon.CurveType)})
	if err != nil {
		return tlv.TLV{}, err
	}
	//Encode internal phonon
	phononTLV, err := backend.TLVEncodePhononDescriptor(&phonon.Phonon)
	if err != nil {
	...
	phononDescriptionTLV, err := tlv.NewTLV(tlv.TagPhononPrivateDescription, data)
	if err != nil {
...
func decodePhononTLV(privatePhononTLV []byte) (phonon MockPhonon, err error) {
	phononTLV, err := tlv.ParseTLVPacket(privatePhononTLV, tlv.TagPhononPrivateDescription)
	if err != nil {
		return phonon, err
	}

	//Parse private key for later
	phonon.PrivateKey, err = phononTLV.FindTag(tlv.TagPhononPrivKey)
	if err != nil {
		log.Debug("could not parse phonon private key tlv")
		return phonon, err
	}
	//Parse standard public fields and extended schema
	publicPhonon, err := backend.TLVDecodePublicPhononFields(phononTLV)
	if err != nil {
		return phonon, err
	}
...
func NewMockCard(isInitialized bool, isStatic bool) (model.PhononCard, error) {
...
func (c *MockCard) LoadCertAuthority(CAPubKey []byte) (err error) {
	if c.caPubKeyLocked {
		return backend.ErrCertLocked
	}
	_, err = util.ParseECCPubKey(CAPubKey)
	if err != nil {
		return backend.ErrInvalidKeyLength
	}
	c.caPubKey = CAPubKey
...
func (c *MockCard) InitCardPairing(receiverCert cert.CardCertificate) (initPairingData []byte, err error) {
	...
	cardCertTLV, err := tlv.NewTLV(tlv.TagCardCertificate, c.IdentityCert.Serialize())
	if err != nil {
		return nil, err
	}
	...
	saltTLV, err := tlv.NewTLV(tlv.TagSalt, c.scPairData.cardToCardSalt)
	if err != nil {
		return nil, err
	}
...
func (c *MockCard) CardPair(initCardPairingData []byte) (cardPairingData []byte, err error) {
	...
	senderCardCertRaw, err := collection.FindTag(tlv.TagCardCertificate)
	if err != nil {
		return nil, errors.New("could not find certificate tlv tag")
	}
	senderSalt, err := collection.FindTag(tlv.TagSalt)
	if err != nil {
	...
	receiverSaltTLV, _ := tlv.NewTLV(tlv.TagSalt, receiverSalt)
	aesIVTLV, _ := tlv.NewTLV(tlv.TagAesIV, aesIV)
	receiverSigTLV, _ := tlv.NewTLV(tlv.TagECDSASig, receiverSig)
...
func (c *MockCard) CardPair2(cardPairData []byte) (cardPair2Data []byte, err error) {
	...
	receiverSalt, err := collection.FindTag(tlv.TagSalt)
	if err != nil {
		return nil, err
	}
	aesIV, err := collection.FindTag(tlv.TagAesIV)
	if err != nil {
		return nil, err
	}
	receiverSig, err := collection.FindTag(tlv.TagECDSASig)
	if err != nil {
		return nil, err
	}
	...
	senderSigTLV, err := tlv.NewTLV(tlv.TagECDSASig, senderSig)
	if err != nil {
		return nil, err
	}
...
func (c *MockCard) FinalizeCardPair(cardPair2Data []byte) (err error) {
	...
	tlvc, err := tlv.ParseTLVPacket(cardPair2Data)
	if err != nil {
		return err
	}
	senderSig, err := tlvc.FindTag(tlv.TagECDSASig)
	if err != nil {
		return err
	}
	//Validate SenderSig
...
func (c *MockCard) SendPhonons(keyIndices []model.PhononKeyIndex, extendedRequest bool) (transferPhononPackets []byte, err error) {
	...
	phononTransferTLV, err := tlv.NewTLV(tlv.TagTransferPhononPacket, outgoingPhonons)
	if err != nil {
...
func (c *MockCard) ReceivePhonons(transaction []byte) (err error) {
	...
	phononTransferPacketTLV, err := tlv.ParseTLVPacket(phononTransferPacketData, tlv.TagTransferPhononPacket)
	if err != nil {
		return err
	}

	phononTLVs, err := phononTransferPacketTLV.FindTags(tlv.TagPhononPrivateDescription)
	if err != nil {
		return err
	}
...
func (c *MockCard) GenerateInvoice() (invoiceData []byte, err error) {
	...
	keyTLV, err := tlv.NewTLV(tlv.TagAESKey, invoiceKey)
	if err != nil {
		return nil, err
	}
	idTLV, err := tlv.NewTLV(tlv.TagAesIV, []byte(invoiceID))
	if err != nil {
		return nil, err
	}
...
func (c *MockCard) ReceiveInvoice(invoiceData []byte) (err error) {
	...
	invoiceKey, err := collection.FindTag(tlv.TagAESKey)
	if err != nil {
		return err
	}
	invoiceID, err := collection.FindTag(tlv.TagAesIV)
	if err != nil {
		return err
	}
...
func (c *MockCard) MineNativePhonon(difficulty uint8) (model.PhononKeyIndex, []byte, error) {
	...
	if !correctDifficulty(pubKey.Hash, int(difficulty)) {
		return model.PhononKeyIndex(0), nil, backend.ErrMiningFailed
	}
